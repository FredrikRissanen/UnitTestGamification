...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" 
  ...
  <xsl:template name="summary">
    ...
    <xsl:variable name="pointCount" select="($testCount - $failureCount - $errorCount)" />
    ...
    <table class="details" border="0" cellpadding="5" cellspacing="2">
      ...
      <tr valign="top">
        ...
        <td>
    		<xsl:call-template name="display-points">
    			<xsl:with-param name="value" select="$pointCount" />
    		</xsl:call-template>    
        </td>
         

        <xsl:if test="$pointCount >= 2">
        <td>
        	<span>Bug explorer</span>
        </td>
        </xsl:if>
        <xsl:if test="$pointCount >= 4">
        <td>
        	<span>Bug finder</span>
        </td>
        </xsl:if>
        <xsl:if test="$pointCount >= 6">
        <td>
        	<span>Bug slayer</span>
        </td>
        </xsl:if>
        <xsl:if test="$pointCount >= 8">
        <td>
        	<span>Bug destroyer</span>
        </td>
        </xsl:if>
        <xsl:if test="$pointCount >= 10">
        <td>
        	<span>Bug master</span>
        </td>
        </xsl:if>
  ...
  <xsl:template name="display-points">
  	<xsl:param name="value"/>
